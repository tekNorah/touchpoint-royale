...
def main(invoice_date):
    ...
    try:
        ...
        invoice_date_prefix = invoice_date.strftime("%Y%m%d")

        # Prefix the Journals to expenses DataFrame name with the formatted invoice date
        new_df_name = f"{invoice_date_prefix}-Journals to expenses"
        ...
        data_frames['Sales Receipts'].rename(columns={'Product/Service Amount': 'Original Product/Service Amount'}, inplace=True)

        # Pivot the specified columns to rows for Sales Receipts DataFrame
        sales_receipts = data_frames['Sales Receipts'].melt(
        ...
        data_frames['Sales Receipts'] = sales_receipts

        # Remove columns not needed from Sales Receipts DataFrame
        data_frames['Sales Receipts'] = data_frames['Sales Receipts'].drop(columns=['Original Product/Service Amount'])

        # Update all rows (excluding the first) with empty strings for specific columns
        data_frames['Sales Receipts'].loc[1:, ['Customer', 'Sales Receipt Date', 'Deposit To', 'Payment method']] = ''

        # Set the Sales Receipt Date (YYYY/MMDD)
        data_frames['Sales Receipts']['Sales Receipt No'] = invoice_date.strftime('%Y/%m%d')

        # Set the value of 'Product/Service Class' to an empty string for the last row (chained assignment)
        data_frames['Sales Receipts'].iloc[-1, data_frames['Sales Receipts'].columns.get_loc('Product/Service Class')] = ''

        # Reorder the columns in the Sales Receipts DataFrame
        data_frames['Sales Receipts'] = data_frames['Sales Receipts'][['Sales Receipt No', 'Customer', 'Sales Receipt Date', 'Deposit To', 'Payment method', 'Global Tax Calculation', 'Product/Service', 'Product/Service Description', 'Product/Service Amount', 'Product/Service Class']]

        # Format the invoice date
        invoice_date_postfix = invoice_date.strftime("%b%d")

        # Postfix the Sales Receipts DataFrame name with the formatted invoice date
        new_df_name = f"Sales Receipts-{invoice_date_postfix}"
        data_frames[new_df_name] = data_frames.pop('Sales Receipts')
